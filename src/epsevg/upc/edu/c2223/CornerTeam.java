...
public class CornerTeam extends TeamRobot{
...
//    }
    ...
    public void run(){
        ...
        int estado = 1;
        //switch (estado) {
        //    case 1:     //asignación de enemigo //hacer pruebas primero con MyFirstDroid, Corner, TeamCorner, MyFirstTeam
                //System.out.println("Monday");
                System.out.println("RADAR EN LA POSICION: "+  getRadarHeading());
                turnRadarRight(360+360);
                //execute();
                System.out.println("RADAR EN LA POSICION DESPUES DE EXECUTE: "+  getRadarHeading());
                System.out.println("__________________________TERMINANDO DE EJECUTAR ___________________________");
                asignarEnemigo();

          /*      break;
            case 2:     //dirigirse enemigo
    ...
    public void asignarEnemigo(){
        //girar el radar para dectetar enemigos
        
        // setTurnRadarRight(360);
        // execute();

        System.out.println("XXXXXXXXXXXXXXX --> enemigos.isEmpty()?  :  " +enemigos.isEmpty());
        if((!enemigos.isEmpty()) && ("epsevg.upc.edu.c2223.CornerTeam* (1)".equals(getName()))){  //Si soy el primer robot
            print(enemigos);
            enemigo_cercano = enemigos.get(0);
            try{
                broadcastMessage("enemigoCercano:"+ enemigo_cercano.name);
            }catch(IOException ignored){}
            
            System.out.println("&&&&&&&&&&&&&& ENEMIGO CERCANO: " + enemigo_cercano.name + "  A DISTANCIA: "
                    + enemigo_cercano.distancia);
           
        }
        else{
            //while(enemigo_cercano != null)
            asignarEnemigosII();
        }
    ...
    public void asignarEnemigosII(){
        while("".equals(EnemigoRecibido)){   
        }
        if (!enemigos.isEmpty() && !"".equals(EnemigoRecibido)){
            
            print(enemigos);
            //mensaje recibido DONE OnMessageRecived
            
            //eliminarlo del array
            remove(enemigos, EnemigoRecibido);

            //mirar enemigo cercano 
            enemigo_cercano = enemigos.get(0);
